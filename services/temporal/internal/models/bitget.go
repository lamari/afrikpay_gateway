...
type BitgetConfig struct {
	...
	SecretKey  string        `yaml:"api_secret" json:"secret_key" validate:"required"`
	Passphrase string        `yaml:"passphrase" json:"passphrase" validate:"required"`
	Timeout    time.Duration `yaml:"timeout" json:"timeout" validate:"required"`
	MaxRetries int           `yaml:"rate_limit" json:"max_retries" validate:"min=0,max=10"`
}
...
// parseRequestTime converts interface{} to int64 for timestamp handling
func parseRequestTime(rt interface{}) int64 {
	switch v := rt.(type) {
	case int64:
		return v
	case float64:
...
func (r *BitgetQuotesResponse) ToQuotesResponse() *QuotesResponse {
	...
	for i, data := range r.Data {
		...
		quotes[i] = QuoteResponse{
			...
			LastPrice: lastPrice,
			Volume:    volume,
			Timestamp: time.Unix(parseRequestTime(r.RequestTime)/1000, 0),
		}
	}
